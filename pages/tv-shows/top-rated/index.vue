...
export default {
  ...
  asyncData({ store }) {
    if (store.state.tvTopRated === null) {
      return axios
        .get(
        ...
        .then(res => {
          ...
          topRatedTvArray.push(...res.data.results);
          store.commit("getTvTopRated", topRatedTvArray);
  ...
  mounted() {
    if (!this.$store.state.tvTopRated[0].runtime) {
      for (const key in this.$store.state.tvTopRated) {
        axios
          .get(
            "tv/" +
              this.movies[key].id +
