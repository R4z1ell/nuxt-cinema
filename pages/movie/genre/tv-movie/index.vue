...
export default {
  ...
  asyncData({ store }) {
    if (store.state.tvMovie === null) {
      return axios
        .get(
          "discover/movie?api_key=" +
            process.env.apiKey +
            "&language=en-US&sort_by=popularity.desc&include_adult=true&include_video=false&page=1&with_genres=10770"
        )
        .then(res => {
          const tvMovieArray = [];
          tvMovieArray.push(...res.data.results);
          store.commit("getTvMovie", tvMovieArray);
  ...
  mounted() {
    if (!this.$store.state.tvMovie[0].runtime) {
      for (const key in this.$store.state.tvMovie) {
        axios
          .get(
            "movie/" +
              this.movies[key].id +
