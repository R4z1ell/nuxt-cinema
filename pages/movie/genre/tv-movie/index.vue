...
export default {
  ...
  computed: {
    movies() {
      return this.$store.state.tvMovie;
    },
    view() {
  ...
  methods: {
    nextPage() {
      axios
        .get(
          "discover/movie?api_key=" +
            ...
            this.$store.state.currentPageTvMovie +
            "&with_genres=10770"
        )
        .then(res => {
          this.$store.commit("pushTvMovie", res.data.results);
      ...
      this.$store.commit("setCurrentPageTvMovie");
    }
  },
  mounted() {
    if (this.$store.state.firstLoadTvMovie) {
      for (const key in this.movies) {
        axios
          ...
          .then(res => {
            ...
            this.$store.commit("setInfoMovie", infoArray);
            this.$store.commit("setFirstLoadTvMovie");
          })
          .catch(error => {
            console.log(error);
          });
