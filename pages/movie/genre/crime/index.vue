...
export default {
  ...
  asyncData({ store }) {
    if (store.state.crime === null) {
      return axios
        .get(
          "discover/movie?api_key=" +
            process.env.apiKey +
            "&language=en-US&sort_by=popularity.desc&include_adult=true&include_video=false&page=1&with_genres=80"
        )
        .then(res => {
          const crimeArray = [];
          crimeArray.push(...res.data.results);
          store.commit("getCrime", crimeArray);
  ...
  mounted() {
    if (!this.$store.state.crime[0].runtime) {
      for (const key in this.$store.state.crime) {
        axios
          .get(
            "movie/" +
              this.movies[key].id +
