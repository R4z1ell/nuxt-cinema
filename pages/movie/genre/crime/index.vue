...
export default {
  ...
  methods: {
    nextPage() {
      axios
        .get(
          "discover/movie?api_key=" +
            ...
            this.$store.state.currentPageCrime +
            "&with_genres=80"
        )
        .then(res => {
          this.$store.commit("pushCrime", res.data.results);
      ...
      this.$store.commit("setCurrentPageCrime");
    }
  },
  mounted() {
    if (this.$store.state.firstLoadCrime) {
      for (const key in this.movies) {
        axios
          ...
          .then(res => {
            ...
            this.$store.commit("setInfoMovie", infoArray);
            this.$store.commit("setFirstLoadCrime");
          })
          .catch(error => {
            console.log(error);
          });
