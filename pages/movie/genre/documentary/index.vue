...
export default {
  ...
  asyncData({ store }) {
    if (store.state.documentary === null) {
      return axios
        .get(
        ...
        .then(res => {
          ...
          documentaryArray.push(...res.data.results);
          store.commit("getDocumentary", documentaryArray);
  ...
  mounted() {
    if (!this.$store.state.documentary[0].runtime) {
      for (const key in this.$store.state.documentary) {
        axios
          .get(
            "movie/" +
              this.movies[key].id +
