...
export default {
  ...
  methods: {
    nextPage() {
      axios
        .get(
          "discover/movie?api_key=" +
            ...
            this.$store.state.currentPageFamily +
            "&with_genres=10751"
        )
        .then(res => {
          this.$store.commit("pushFamily", res.data.results);
      ...
      this.$store.commit("setCurrentPageFamily");
    }
  },
  mounted() {
    if (this.$store.state.firstLoadFamily) {
      for (const key in this.movies) {
        axios
          ...
          .then(res => {
            ...
            this.$store.commit("setInfoMovie", infoArray);
            this.$store.commit("setFirstLoadFamily");
          })
          .catch(error => {
            console.log(error);
          });
