...
export default {
  ...
  asyncData({ store }) {
    if (store.state.family === null) {
      return axios
        .get(
          "discover/movie?api_key=" +
            process.env.apiKey +
            "&language=en-US&sort_by=popularity.desc&include_adult=true&include_video=false&page=1&with_genres=10751"
        )
        .then(res => {
          const familyArray = [];
          familyArray.push(...res.data.results);
          store.commit("getFamily", familyArray);
  ...
  mounted() {
    if (!this.$store.state.family[0].runtime) {
      for (const key in this.$store.state.family) {
        axios
          .get(
            "movie/" +
              this.movies[key].id +
