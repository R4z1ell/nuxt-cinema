...
export default {
  ...
  asyncData({ store }) {
    if (store.state.drama === null) {
      return axios
        .get(
          "discover/movie?api_key=" +
            process.env.apiKey +
            "&language=en-US&sort_by=popularity.desc&include_adult=true&include_video=false&page=1&with_genres=18"
        )
        .then(res => {
          const dramaArray = [];
          dramaArray.push(...res.data.results);
          store.commit("getDrama", dramaArray);
  ...
  mounted() {
    if (!this.$store.state.drama[0].runtime) {
      for (const key in this.$store.state.drama) {
        axios
          .get(
            "movie/" +
              this.movies[key].id +
