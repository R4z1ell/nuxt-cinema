...
export default {
  ...
  methods: {
    nextPage() {
      axios
        .get(
          "discover/movie?api_key=" +
            ...
            this.$store.state.currentPageDrama +
            "&with_genres=18"
        )
        .then(res => {
          this.$store.commit("pushDrama", res.data.results);
      ...
      this.$store.commit("setCurrentPageDrama");
    }
  },
  mounted() {
    if (this.$store.state.firstLoadDrama) {
      for (const key in this.movies) {
        axios
          ...
          .then(res => {
            ...
            this.$store.commit("setInfoMovie", infoArray);
            this.$store.commit("setFirstLoadDrama");
          })
          .catch(error => {
            console.log(error);
          });
