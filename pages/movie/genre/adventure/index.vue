...
export default {
  ...
  asyncData({ store }) {
    if (store.state.adventure === null) {
      return axios
        .get(
          "discover/movie?api_key=" +
            process.env.apiKey +
            "&language=en-US&sort_by=popularity.desc&include_adult=true&include_video=false&page=1&with_genres=12"
        )
        .then(res => {
          const adventureArray = [];
          adventureArray.push(...res.data.results);
          store.commit("getAdventure", adventureArray);
  ...
  mounted() {
    if (!this.$store.state.adventure[0].runtime) {
      for (const key in this.$store.state.adventure) {
        axios
          .get(
            "movie/" +
              this.movies[key].id +
