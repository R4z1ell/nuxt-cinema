...
export default {
  ...
  computed: {
    movies() {
      return this.$store.state.adventure;
    },
    view() {
  ...
  methods: {
    nextPage() {
      axios
        .get(
          "discover/movie?api_key=" +
            ...
            this.$store.state.currentPageAdventure +
            "&with_genres=12"
        )
        .then(res => {
          this.$store.commit("pushAdventure", res.data.results);
      ...
      this.$store.commit("setCurrentPageAdventure");
    }
  },
  mounted() {
    if (this.$store.state.firstLoadAdventure) {
      for (const key in this.movies) {
        axios
          ...
          .then(res => {
            ...
            this.$store.commit("setInfoMovie", infoArray);
            this.$store.commit("setFirstLoadAdventure");
          })
          .catch(error => {
            console.log(error);
          });
