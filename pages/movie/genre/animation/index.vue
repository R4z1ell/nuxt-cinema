...
export default {
  ...
  asyncData({ store }) {
    if (store.state.animation === null) {
      return axios
        .get(
          "discover/movie?api_key=" +
            process.env.apiKey +
            "&language=en-US&sort_by=popularity.desc&include_adult=true&include_video=false&page=1&with_genres=16"
        )
        .then(res => {
          const animationArray = [];
          animationArray.push(...res.data.results);
          store.commit("getAnimation", animationArray);
  ...
  mounted() {
    if (!this.$store.state.animation[0].runtime) {
      for (const key in this.$store.state.animation) {
        axios
          .get(
            "movie/" +
              this.movies[key].id +
