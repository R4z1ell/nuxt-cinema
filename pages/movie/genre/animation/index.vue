...
export default {
  ...
  methods: {
    nextPage() {
      axios
        .get(
          "discover/movie?api_key=" +
            ...
            this.$store.state.currentPageAnimation +
            "&with_genres=16"
        )
        .then(res => {
          this.$store.commit("pushAnimation", res.data.results);
      ...
      this.$store.commit("setCurrentPageAnimation");
    }
  },
  mounted() {
    if (this.$store.state.firstLoadAnimation) {
      for (const key in this.movies) {
        axios
          ...
          .then(res => {
            ...
            this.$store.commit("setInfoMovie", infoArray);
            this.$store.commit("setFirstLoadAnimation");
          })
          .catch(error => {
            console.log(error);
          });
