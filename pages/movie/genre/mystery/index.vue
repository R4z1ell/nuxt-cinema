...
export default {
  ...
  asyncData({ store }) {
    if (store.state.mystery === null) {
      return axios
        .get(
          "discover/movie?api_key=" +
            process.env.apiKey +
            "&language=en-US&sort_by=popularity.desc&include_adult=true&include_video=false&page=1&with_genres=9648"
        )
        .then(res => {
          const mysteryArray = [];
          mysteryArray.push(...res.data.results);
          store.commit("getMystery", mysteryArray);
  ...
  mounted() {
    if (!this.$store.state.mystery[0].runtime) {
      for (const key in this.$store.state.mystery) {
        axios
          .get(
            "movie/" +
              this.movies[key].id +
