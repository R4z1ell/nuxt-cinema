...
export default {
  ...
  asyncData({ store }) {
    if (store.state.western === null) {
      return axios
        .get(
          "discover/movie?api_key=" +
            process.env.apiKey +
            "&language=en-US&sort_by=popularity.desc&include_adult=true&include_video=false&page=1&with_genres=37"
        )
        .then(res => {
          const westernArray = [];
          westernArray.push(...res.data.results);
          store.commit("getWestern", westernArray);
  ...
  mounted() {
    if (!this.$store.state.western[0].runtime) {
      for (const key in this.$store.state.western) {
        axios
          .get(
            "movie/" +
              this.movies[key].id +
