...
export default {
  ...
  computed: {
    movies() {
      return this.$store.state.western;
    },
    view() {
  ...
  methods: {
    nextPage() {
      axios
        .get(
          "discover/movie?api_key=" +
            ...
            this.$store.state.currentPageWestern +
            "&with_genres=37"
        )
        .then(res => {
          this.$store.commit("pushWestern", res.data.results);
      ...
      this.$store.commit("setCurrentPageWestern");
    }
  },
  mounted() {
    if (this.$store.state.firstLoadWestern) {
      for (const key in this.movies) {
        axios
          ...
          .then(res => {
            ...
            this.$store.commit("setInfoMovie", infoArray);
            this.$store.commit("setFirstLoadWestern");
          })
          .catch(error => {
            console.log(error);
          });
