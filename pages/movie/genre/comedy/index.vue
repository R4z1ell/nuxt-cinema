...
export default {
  ...
  asyncData({ store }) {
    if (store.state.comedy === null) {
      return axios
        ...
        .then(res => {
          const comedyArray = [];
          comedyArray.push(...res.data.results);
          store.commit("getComedy", comedyArray);
  ...
  mounted() {
    if (!this.$store.state.comedy[0].runtime) {
      for (const key in this.$store.state.comedy) {
        axios
          .get(
            "movie/" +
              this.movies[key].id +
