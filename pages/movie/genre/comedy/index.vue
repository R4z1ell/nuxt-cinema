...
export default {
  ...
  computed: {
    movies() {
      return this.$store.state.comedy;
    },
    view() {
  ...
  methods: {
    nextPage() {
      axios
        .get(
          "discover/movie?api_key=" +
            ...
            this.$store.state.currentPageComedy +
            "&with_genres=35"
        )
        .then(res => {
          this.$store.commit("pushComedy", res.data.results);
      ...
      this.$store.commit("setCurrentPageComedy");
    }
  },
  mounted() {
    if (this.$store.state.firstLoadComedy) {
      for (const key in this.movies) {
        axios
          ...
          .then(res => {
            ...
            this.$store.commit("setInfoMovie", infoArray);
            this.$store.commit("setFirstLoadComedy");
          })
          .catch(error => {
            console.log(error);
          });
