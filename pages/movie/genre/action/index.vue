...
export default {
  ...
  asyncData({ store }) {
    if (store.state.action === null) {
      return axios
        ...
        .then(res => {
          const actionArray = [];
          actionArray.push(...res.data.results);
          store.commit("getAction", actionArray);
  ...
  mounted() {
    if (!this.$store.state.action[0].runtime) {
      for (const key in this.$store.state.action) {
        axios
          .get(
            "movie/" +
              this.movies[key].id +
