...
export default {
  ...
  methods: {
    nextPage() {
      axios
        .get(
          "discover/movie?api_key=" +
            ...
            this.$store.state.currentPageAction +
            "&with_genres=28"
        )
        .then(res => {
          this.$store.commit("pushAction", res.data.results);
      ...
      this.$store.commit("setCurrentPageAction");
    }
  },
  mounted() {
    if (this.$store.state.firstLoadAction) {
      for (const key in this.movies) {
        axios
          ...
          .then(res => {
            ...
            this.$store.commit("setInfoMovie", infoArray);
            this.$store.commit("setFirstLoadAction");
          })
          .catch(error => {
            console.log(error);
          });
