...
export default {
  ...
  asyncData({ store }) {
    if (store.state.thriller === null) {
      return axios
        ...
        .then(res => {
          const thrillerArray = [];
          thrillerArray.push(...res.data.results);
          store.commit("getThriller", thrillerArray);
  ...
  mounted() {
    if (!this.$store.state.thriller[0].runtime) {
      for (const key in this.$store.state.thriller) {
        axios
          .get(
            "movie/" +
              this.movies[key].id +
