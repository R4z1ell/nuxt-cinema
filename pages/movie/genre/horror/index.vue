...
export default {
  ...
  asyncData({ store }) {
    if (store.state.horror === null) {
      return axios
        .get(
          "discover/movie?api_key=" +
            process.env.apiKey +
            "&language=en-US&sort_by=popularity.desc&include_adult=true&include_video=false&page=1&with_genres=27"
        )
        .then(res => {
          const horrorArray = [];
          horrorArray.push(...res.data.results);
          store.commit("getHorror", horrorArray);
  ...
  mounted() {
    if (!this.$store.state.horror[0].runtime) {
      for (const key in this.$store.state.horror) {
        axios
          .get(
            "movie/" +
              this.movies[key].id +
