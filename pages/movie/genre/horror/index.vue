...
export default {
  ...
  methods: {
    nextPage() {
      axios
        .get(
          "discover/movie?api_key=" +
            ...
            this.$store.state.currentPageHorror +
            "&with_genres=27"
        )
        .then(res => {
          this.$store.commit("pushHorror", res.data.results);
      ...
      this.$store.commit("setCurrentPageHorror");
    }
  },
  mounted() {
    if (this.$store.state.firstLoadHorror) {
      for (const key in this.movies) {
        axios
          ...
          .then(res => {
            ...
            this.$store.commit("setInfoMovie", infoArray);
            this.$store.commit("setFirstLoadHorror");
          })
          .catch(error => {
            console.log(error);
          });
