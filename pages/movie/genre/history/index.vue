...
export default {
  ...
  asyncData({ store }) {
    if (store.state.history === null) {
      return axios
        .get(
          "discover/movie?api_key=" +
            process.env.apiKey +
            "&language=en-US&sort_by=popularity.desc&include_adult=true&include_video=false&page=1&with_genres=36"
        )
        .then(res => {
          const historyArray = [];
          historyArray.push(...res.data.results);
          store.commit("getHistory", historyArray);
  ...
  mounted() {
    if (!this.$store.state.history[0].runtime) {
      for (const key in this.$store.state.history) {
        axios
          .get(
            "movie/" +
              this.movies[key].id +
