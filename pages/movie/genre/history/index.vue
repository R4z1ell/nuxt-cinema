...
export default {
  ...
  methods: {
    nextPage() {
      axios
        .get(
          "discover/movie?api_key=" +
            ...
            this.$store.state.currentPageHistory +
            "&with_genres=36"
        )
        .then(res => {
          this.$store.commit("pushHistory", res.data.results);
      ...
      this.$store.commit("setCurrentPageHistory");
    }
  },
  mounted() {
    if (this.$store.state.firstLoadHistory) {
      for (const key in this.movies) {
        axios
          ...
          .then(res => {
            ...
            this.$store.commit("setInfoMovie", infoArray);
            this.$store.commit("setFirstLoadHistory");
          })
          .catch(error => {
            console.log(error);
          });
