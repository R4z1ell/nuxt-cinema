...
export default {
  ...
  asyncData({ store }) {
    if (store.state.romance === null) {
      return axios
        .get(
          "discover/movie?api_key=" +
            process.env.apiKey +
            "&language=en-US&sort_by=popularity.desc&include_adult=true&include_video=false&page=1&with_genres=10749"
        )
        .then(res => {
          const romanceArray = [];
          romanceArray.push(...res.data.results);
          store.commit("getRomance", romanceArray);
  ...
  mounted() {
    if (!this.$store.state.romance[0].runtime) {
      for (const key in this.$store.state.romance) {
        axios
          .get(
            "movie/" +
              this.movies[key].id +
