...
export default {
  ...
  computed: {
    movies() {
      return this.$store.state.fantasy;
    },
    view() {
  ...
  methods: {
    nextPage() {
      axios
        .get(
          "discover/movie?api_key=" +
            ...
            this.$store.state.currentPageFantasy +
            "&with_genres=14"
        )
        .then(res => {
          this.$store.commit("pushFantasy", res.data.results);
      ...
      this.$store.commit("setCurrentPageFantasy");
    }
  },
  mounted() {
    if (this.$store.state.firstLoadFantasy) {
      for (const key in this.movies) {
        axios
          ...
          .then(res => {
            ...
            this.$store.commit("setInfoMovie", infoArray);
            this.$store.commit("setFirstLoadFantasy");
          })
          .catch(error => {
            console.log(error);
          });
