...
export default {
  ...
  asyncData({ store }) {
    if (store.state.fantasy === null) {
      return axios
        ...
        .then(res => {
          const fantasyArray = [];
          fantasyArray.push(...res.data.results);
          store.commit("getFantasy", fantasyArray);
  ...
  mounted() {
    if (!this.$store.state.fantasy[0].runtime) {
      for (const key in this.$store.state.fantasy) {
        axios
          .get(
            "movie/" +
              this.movies[key].id +
