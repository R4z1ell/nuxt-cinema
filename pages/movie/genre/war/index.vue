...
export default {
  ...
  asyncData({ store }) {
    if (store.state.war === null) {
      return axios
        .get(
          "discover/movie?api_key=" +
            process.env.apiKey +
            "&language=en-US&sort_by=popularity.desc&include_adult=true&include_video=false&page=1&with_genres=10752"
        )
        .then(res => {
          const warArray = [];
          warArray.push(...res.data.results);
          store.commit("getWar", warArray);
  ...
  mounted() {
    if (!this.$store.state.war[0].runtime) {
      for (const key in this.$store.state.war) {
        axios
          .get(
            "movie/" +
              this.movies[key].id +
