...
export default {
  ...
  methods: {
    nextPage() {
      axios
        .get(
          "discover/movie?api_key=" +
            ...
            this.$store.state.currentPageWar +
            "&with_genres=10752"
        )
        .then(res => {
          this.$store.commit("pushWar", res.data.results);
      ...
      this.$store.commit("setCurrentPageWar");
    }
  },
  mounted() {
    if (this.$store.state.firstLoadWar) {
      for (const key in this.movies) {
        axios
          ...
          .then(res => {
            ...
            this.$store.commit("setInfoMovie", infoArray);
            this.$store.commit("setFirstLoadWar");
          })
          .catch(error => {
            console.log(error);
          });
